...
from .core.oauth_config import oauth_settings  # ✅ NEW: OAuth settings
from .core.database import get_db, check_db_connection, get_db_info
from .api import customers, events, photos, auth  # ✅ NEW: Auth router toegevoegd
from .schemas.customer import HealthCheck

# Create FastAPI app
app = FastAPI(
    title=settings.APP_NAME,
    version="2.0.0",  # ✅ UPDATED: OAuth version
    description="API for Social Media Poster - Manage customers, events, photos, and automated posts with OAuth 2.0 authentication",
    debug=settings.DEBUG
)

# ✅ UPDATED: Configure CORS with OAuth settings
app.add_middleware(
    CORSMiddleware,
...
# ✅ UPDATED: Include routers (auth router first!)
app.include_router(auth.router, prefix=settings.API_V1_PREFIX)      # ✅ NEW: Auth endpoints
...
def read_root():
    """API information"""
    return {
        "message": "Welcome to Social Media Poster API with OAuth 2.0",
        "version": "2.0.0",
        "authentication": "OAuth 2.0 / Google",  # ✅ NEW
        "docs_url": "/docs",
        "redoc_url": "/redoc",
        "health_url": "/health",
        "auth_health_url": f"{settings.API_V1_PREFIX}/auth/health",  # ✅ NEW
        "endpoints": {
            "auth": f"{settings.API_V1_PREFIX}/auth",  # ✅ NEW
...
# ✅ NEW: OAuth health check endpoint
@app.get("/auth-health")
...
async def startup_event():
    ...
    print(f"\n{'='*60}")
    print(f"🚀 Starting {settings.APP_NAME} v2.0.0 (OAuth)")
    print(f"{'='*60}")
...
async def shutdown_event():
    ...
    print("\n👋 Shutting down Social Media Poster API (OAuth version)")
